...
random_perturbations: A 2D array where each row is a perturbation of the original signal.
original_sig_rep: A 2D array representing the original signal.


pairwise_distances computes the cosine distances between each row of random_perturbations and original_sig_rep.
The result is a 2D array where each element represents the cosine distance between a perturbation and the original signal.
Flattening the Distance Matrix:

ravel() converts the 2D distance matrix to a 1D array.
'''
...
def calculate_cosine_distances(random_perturbations, num_slices):
    ...
    """
    # Represent the original signal as a perturbation where all segments are enabled (i.e., a vector of ones)
    original_sig_rep = np.ones((1, num_slices))

    # Calculate cosine distances
    cosine_distances = pairwise_distances(random_perturbations, original_sig_rep, metric='cosine').ravel()
...
def segment_signal(instance_sig, num_slices=40):
    """
    Segments the signal into a fixed number of slices.

    Parameters:
        instance_sig (np.ndarray): The signal instance to segment.
        num_slices (int): The number of slices to divide the signal into.

    Returns:
        int: The width of each slice in the segmented signal.
    """
    total_length = len(instance_sig)
...
def generate_random_perturbations(num_perturbations, num_slices):
    """
    Generates random perturbations for signal segments.
    
    This function creates a binary matrix where each row represents a perturbation,
    and each column corresponds to a segment of the  signal. A value of '1' indicates
    ...
    Parameters:
        ...
        num_slices (int): The number of slices (segments) each  signal is divided into.
...
def predict_perturbations(model, instance_sig, random_perturbations, num_slices, perturb_function):
    """
    ...
    Returns:
    - np.ndarray: An array of model predictions for each perturbed  signal.
    """
...
def calculate_cosine_distances(random_perturbations, num_slices):
    ...
    Parameters:
    - random_perturbations (np.ndarray): An array of perturbation vectors.
    - num_slices (int): The total number of segments the signal is divided into, matching the dimension of the perturbations.
...
def identify_top_influential_segments(segment_importance_coefficients, number_of_top_features=5):
    """
    Identifies the top influential segments of the signal based on the importance coefficients 
